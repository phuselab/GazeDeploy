...
parser.add_argument("--iou_thres", type=float, default=0.5,
                    help="Minimum intersection over union between consecutive face detections"
                    )
parser.add_argument("--num_fail", type=int, default=3, help="Number of missed face detections allowed")
parser.add_argument("--min_size", type=float, default=0.05, help="Minimum size of faces for face tracking")
...
def track_shot(opt,scenefaces):
  ...
  # minSize   = 0.05    # Minimum size of faces  iouThres  = 0.5

  iouThres  = opt.iou_thres
  numFail   = opt.num_fail
...
def crop_video(opt,track,cropfile):
  ...
  command = ("ffmpeg -y -nostdin -i %s -ac 1 -vn -acodec pcm_s16le -ar 16000 -ss %.3f -to %.3f %s" % (os.path.join(opt.avi_dir,opt.reference,'video.avi'),audiostart,audioend,audiotmp)) #-async 1
  ...
  command = ("ffmpeg -y -nostdin -i %st.avi -i %s -c:v copy -c:a copy %s.avi" % (cropfile,audiotmp,cropfile)) #-async 1
  output = subprocess.call(command, shell=True, stdout=None)
...
def inference_video(opt):
  ...
  PATH_TO_CKPT = 'speaker_detect/protos/frozen_inference_graph_face.pb'

  # List of the strings that is used to add correct label for each box.
  PATH_TO_LABELS = 'speaker_detect/protos/face_label_map.pbtxt'
...
def scene_detect(opt):
  ...
  video_file_path = os.path.join(opt.avi_dir,opt.reference,'video.avi')
  ...
  #
  # video_framerate, frames_read = scenedetect.detect_scenes_file(video_file_path, scene_list, detector_list)
  ###################################

  # NEW
  # This change enables us to use the 0.5+ version of scenedetect instead of the old one (0.3.5)
  ...
  try:
    ...
    video_manager.start()
    frames_read = scene_manager.detect_scenes(frame_source=video_manager)
    scene_list = scene_manager.get_scene_list(base_timecode)
    scene_list = [
      (scene[0].get_frames()) for scene in scene_list
    ]
  finally:
...
# There might be a problem here of audio going out of sync, don't know why though...
#command = ("ffmpeg -y -nostdin -i %s -qscale:v 4 -async 1 -deinterlace %s" % (opt.videofile,os.path.join(opt.avi_dir,opt.reference,'video.avi'))) #-async 1
command = ("ffmpeg -y -nostdin -i %s -qscale:v 4 -async 1 -r 25 -deinterlace %s" % (opt.videofile,os.path.join(opt.avi_dir,opt.reference,'video.avi'))) #-async 1
output = subprocess.call(command, shell=True, stdout=None)
